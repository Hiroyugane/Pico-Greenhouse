...
Cross-cutting rules — clarifying questions, commit conventions, and the documentation routine — come from the **global ruleset** (`~/.claude/rules/`, auto-loaded every session) plus the ECC plugin; they are no longer vendored into this repo. Two project-specific rule files remain local, and the global routines have project-specific targets here:

- **Documentation routine** (global `documentation-routine.md`) — this repo's two rolling logs are [docs/test/hw-test-log.md](docs/test/hw-test-log.md) (hardware / eyes-on verification, since most of this system can't be exercised by `pytest`) and [docs/notes/chat-log.md](docs/notes/chat-log.md) (decisions, spec clarifications, deviations, issues, notes). Append the same turn the change lands; mention both in the end-of-turn summary.
- **Commit conventions** (global `commit-conventions.md`) — one logical change per commit; Conventional-Commits subject with a body stating WHAT and WHY; refactor and behavior changes split; tests ship with the code they cover; no `wip` / `checkpoint` / grab-bag commits; no squash-merging.
- [.claude/rules/configurability.md](.claude/rules/configurability.md) — every tunable value lives in `DEVICE_CONFIG` with a `validate_config()` entry and a `tests/test_config.py` row, added in the same commit.
- [.claude/rules/case-legend.md](.claude/rules/case-legend.md) — the printed enclosure legend ([docs/hardware/case-legend.pdf](docs/hardware/case-legend.pdf), generated by [tools/gen_case_legend.py](tools/gen_case_legend.py)) must be updated and regenerated the same turn any operator-visible surface changes: OLED pages/fields, relay or PWM channel assignment, LED policy, alert keys, button timings.
- [.claude/rules/hardware-revision-notes.md](.claude/rules/hardware-revision-notes.md) — any finding the next PCB / enclosure / wiring revision must absorb (component swap, footprint change, missing bulk cap, mounting fix, harness rework) appends to [docs/hardware/next-revision.md](docs/hardware/next-revision.md) the same turn, backed by a chat-log entry, a memory file, and an hw-test-log checklist when post-fab verification applies.
