...
Cross-cutting conventions for working on this repo:

- **Working logs live outside the repo.** The rolling decision log and the hardware/eyes-on test log are maintained privately (see `CLAUDE.local.md` if present for their locations). Do not create `docs/notes/chat-log.md` or `docs/test/hw-test-log.md` in the repo — they were deliberately removed before publication.
- **Commit conventions** — one logical change per commit; Conventional-Commits subject with a body stating WHAT and WHY; refactor and behavior changes split; tests ship with the code they cover; no `wip` / `checkpoint` / grab-bag commits; no squash-merging.
- **Configurability** — every tunable value lives in `DEVICE_CONFIG` with a `validate_config()` entry and a `tests/test_config.py` row, added in the same commit.
- **Case legend** — the printed enclosure legend ([docs/hardware/case-legend.pdf](docs/hardware/case-legend.pdf), generated by [tools/gen_case_legend.py](tools/gen_case_legend.py)) must be updated and regenerated the same turn any operator-visible surface changes: OLED pages/fields, relay or PWM channel assignment, LED policy, alert keys, button timings.
- **Hardware revision notes** — any finding the next PCB / enclosure / wiring revision must absorb (component swap, footprint change, missing bulk cap, mounting fix, harness rework) appends to [docs/hardware/next-revision.md](docs/hardware/next-revision.md) the same turn.
