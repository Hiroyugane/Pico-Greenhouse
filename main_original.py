# Pi Greenhouse - Main Orchestrator
# Dennis Hiro, 2024-06-08
# Ver: InDev2.0 (Refactored: Modular Architecture)
#
# Clean entry point that:
# 1. Validates configuration
# 2. Initializes hardware (RTC, SPI, SD, GPIO)
# 3. Creates loggers and controllers via dependency injection
# 4. Spawns async tasks for continuous operation
#
# HOW TO RUN:
# 1. First time only: run rtc_set_time.py to sync RTC
# 2. Run this main.py via Thonny
# 3. Check /sd/dht_log_YYYY-MM-DD.csv for data after runtime
#
# Hardware Pinout (from config.py):
# - DHT22: GPIO 15
# - RTC: I2C (GPIO 0 SDA, GPIO 1 SCL)
# - SD Card: SPI (GPIO 10-13)
# - Relays: GPIO 16, 17, 18
# - LEDs: GPIO 25 (status), 24 (reminder)
# - Button: GPIO 23 (reminder reset)
...
async def main():
    ...
    2. Initialize hardware (factory pattern)
    3. Create TimeProvider wrapper around RTC
    4. Create BufferManager for resilient storage
    5. Create EventLogger for system logging
    6. Create DHTLogger for sensor data
    7. Create relay controllers (fans, grow light)
    8. Create LED/button handler and Service reminder
    9. Spawn all async tasks
    ...
    buffer_manager = BufferManager(
        ...
        flush_threshold=buffer_config.get('flush_threshold', 50),
    )
    ...
    dht_logger = DHTLogger(
        ...
        max_buffer_size=dht_config.get('max_buffer_size', 200),
    )
    ...
    for pin, config, name in fan_configs:
        ...
        fan_tasks.append(asyncio.create_task(fan.start_cycle()))
    ...
    while True:
        ...
        # Attempt to migrate fallback if primary became available
        if metrics['writes_to_fallback'] > metrics['fallback_migrations']:
            migrated = buffer_manager.migrate_fallback()
            if migrated > 0:
                logger.info('MAIN', f'Migrated {migrated} fallback entries to primary')
...
# Delete after verification that new architecture works
# ============================================================================

class EventLogger_OLD:
    """
